"""
Configuration for sec-edgar-mcp server.

This module defines the server configuration, including validation of required
and optional environment variables.

This server uses the free public SEC EDGAR API (data.sec.gov) and does not require
authentication. However, SEC requires a User-Agent header, which we provide by default.
"""
...
@dataclass
class SecEdgarConfig(ServerConfig):
    """
    Configuration for sec-edgar-mcp server.
    
    This server uses the free public SEC EDGAR API. SEC requires a User-Agent
    header, but we provide a default if not specified. All configuration options
    are optional.
    """
    
    # Optional: User-Agent for SEC EDGAR API (required by SEC, but we provide default)
...
def load_config() -> SecEdgarConfig:
    """
    ...
    return SecEdgarConfig(
        sec_user_agent=os.getenv("SEC_USER_AGENT", "MCP SEC EDGAR Server (contact@example.com)"),
